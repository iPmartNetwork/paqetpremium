...
### Fixed
- SOCKS5 UDP tunneling over KCP: client now frames datagrams with length-prefix (`WriteDatagram`/`ReadDatagram`) to match the server relay, fixing broken DNS/games/QUIC-over-SOCKS; QUIC upstreams use the datagram path automatically.
- Installer: client wizard documents SOCKS5 TCP+UDP, prompts bind address, improves UDP port hints; `update` prefers source rebuild when a checkout exists so fixes land before a new release binary is published.
...
[0.18.0]: https://github.com/iPmartNetwork/paqetpremium/releases/tag/v0.18.0
